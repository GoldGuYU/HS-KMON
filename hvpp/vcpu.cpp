  ...
  switch (static_cast<state>(launch_context_.capture()))
  {
    case state::off:
      if (auto err = vmx_enter())
      {
        ...
        //
        _dbg("vcpu->start( ) fail VMX_ENTER");
      ...
      //
      _dbg("vcpu->start( ) FAIL VM_LAUNCH");
...
auto vcpu_t::guest_mtf(bool state) noexcept -> void
{
  auto ctls = processor_based_controls();

  ctls.monitor_trap_flag = state;

  processor_based_controls(ctls);
}

auto vcpu_t::tsc_virt() const noexcept -> uint64_t
{
  return tsc_virt_;
}
...
auto vcpu_t::compensate_tsc() const noexcept -> bool
{
  return compensate_tsc_;
}

auto vcpu_t::compensate_tsc(bool state) noexcept -> void
{
  compensate_tsc_ = state;
}
...
auto vcpu_t::handle_emulated_instruction() noexcept -> void
{
  if (guest_rflags().trap_flag && !guest_debugctl().btf)
  {
    auto pending_debug_exceptions = guest_pending_debug_exceptions();
    pending_debug_exceptions.caused_by_single_step = 1;
    guest_pending_debug_exceptions(pending_debug_exceptions);

    auto interruptibility_state = guest_interruptibility_state();
    interruptibility_state.blocking_by_mov_ss = 0;
    interruptibility_state.blocking_by_smi = 0;
    interruptibility_state.blocking_by_sti = 0;
    guest_interruptibility_state(interruptibility_state);
  }
}
...
auto vcpu_t::setup_host() noexcept -> error_code_t
{
  ...
  //
  // Update host fields to use our custom IDT.
  //

  idtr.base_address = (uintptr_t)g::idt.table;

  g::idt.table[0x0E] = idt::create_entry( asm_interrupt_handler_pf );
...
auto vcpu_t::setup_guest() noexcept -> error_code_t
{
  ...
  const auto setup_msr_bitmap = [](vmx::msr_bitmap_t& msr_bmp)
  {
    EXIT_ON_MSR(msr::efer_t::msr_id);
    EXIT_ON_MSR(msr::debugctl_t::msr_id);
    EXIT_ON_MSR(msr::tsx_ctrl_t::msr_id);
    EXIT_ON_MSR(msr::lbr_select_t::msr_id);
    EXIT_ON_MSR(msr::arch_capabilities_t::msr_id);
    EXIT_ON_MSR(msr::fs_base_t::msr_id);
    EXIT_ON_MSR(msr::gs_base_t::msr_id);
  };

  //
  // By default we won't force VM-exit on any external interrupts.
  // Remember that NMI exiting for some reason causes VMWARE to hang,
  // But works flawlessly on physical hardware like our main computer.
  //
  auto pinbased_ctls = pin_based_controls();
  pin_based_controls(pinbased_ctls);

  auto exception_bmp = exception_bitmap();
  //exception_bmp.invalid_opcode = true;
  exception_bitmap(exception_bmp);
  ...
  //
  auto entry_ctls = vm_entry_controls();
  ...
  auto exit_ctls = vm_exit_controls();
  ...
  //
  auto msr_bmp = vmx::msr_bitmap_t{};
  setup_msr_bitmap(msr_bmp);
  msr_bitmap(msr_bmp);
...
void vcpu_t::entry_host() noexcept
{
...

  {
    ...
    if (state_ == state::terminated)
    {
      _dbg("terminate request has been received!");
      //
    ...
    if (!suppress_rip_adjust_)
    {
      ...
      //
      // Only check for potential #DB handling when advancing RIP.
      //
      handle_emulated_instruction();
    }
